...
class TableFactory implements EweyFactory {
  ...
  constructor(priority: number = 110, restrictToKeys: string[] | null = null, pathMatch: string[] | null = null){
    ...
    if (this.restrictToKeys) {
      for (const key in properties) {
        if (this.restrictToKeys.includes(key)){
          ...
          columns.push({ key, Field: JsonSchemaComponentFactory(properties[key], components, currentPath, factories) })
          currentPath.pop()
        }
      }
    } else {
